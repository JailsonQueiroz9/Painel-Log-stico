  ...
  return (
    <>
      <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 md:p-10 bg-black/95 backdrop-blur-xl animate-in fade-in duration-300">
        <div className="bg-custom-main w-full max-w-5xl rounded-[40px] overflow-hidden shadow-[0_32px_64px_-12px_rgba(0,0,0,0.8)] border border-slate-200 dark:border-white/5 animate-in zoom-in-95 duration-200 flex flex-col md:flex-row h-full max-h-[600px] text-custom-main">
          ...
          <div className="w-full md:w-2/5 bg-gradient-to-br from-[#1e293b] to-[#050a14] flex flex-col items-center justify-center p-12 relative overflow-hidden shrink-0">
             <div className="absolute top-0 left-0 w-full h-full opacity-10 pointer-events-none">
                <div className="absolute -top-24 -left-24 w-80 h-80 bg-primary-600 rounded-full blur-[120px]" />
                <div className="absolute -bottom-24 -right-24 w-80 h-80 bg-indigo-600 rounded-full blur-[120px]" />
             </div>

             <div className="w-48 h-48 bg-primary-600 rounded-[56px] flex items-center justify-center text-white text-8xl font-[900] italic shadow-2xl shadow-primary-600/30 z-10 border border-slate-200 dark:border-white/10 overflow-hidden">
                {user.profileImage ? (
                  <img 
                    ...
                    onError={(e) => {
                      (e.target as HTMLImageElement).src = 'https://ui-avatars.com/api/?name=' + encodeURIComponent(handleName) + '&background=random&size=256';
                    }}
                  />
                ) : (
                  userInitial
          ...
          <div className="flex-1 p-8 md:p-16 flex flex-col justify-between bg-custom-main relative">
             
             {/* Botão de Fechar */}
             <button 
                onClick={onClose} 
                className="absolute top-8 right-8 w-12 h-12 flex items-center justify-center bg-white/5 hover:bg-white/10 text-slate-900 dark:text-white rounded-full transition-all border border-slate-200 dark:border-white/5"
             >
                <X size={20} />
             </button>
             ...
             <div className="space-y-12">
                ...
                <div className="space-y-1">
                   <span className="text-[10px] font-black text-primary-500 uppercase tracking-[0.5em] italic">Registro Funcional</span>
                   <h3 className="text-custom-main text-2xl font-black uppercase tracking-tight">Perfil do Colaborador</h3>
                </div>
                ...
                <div className="space-y-4">
                   ...
                   <p className="text-slate-600 dark:text-slate-300 text-lg leading-relaxed font-bold tracking-tight max-w-2xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                   <div className="flex items-center gap-6 p-6 bg-white dark:bg-slate-900/40 rounded-3xl border border-slate-200 dark:border-slate-800/50 group transition-all hover:bg-white dark:bg-slate-900/60">
                      <div className="w-14 h-14 bg-slate-50 dark:bg-slate-950 rounded-2xl flex items-center justify-center text-slate-600 group-hover:text-primary-500 transition-colors border border-slate-200 dark:border-slate-800">
                        <MapPin size={24} strokeWidth={2.5} />
                      </div>
                      <div className="space-y-1">
                         <p className="text-[9px] font-black text-slate-500 uppercase tracking-[0.4em] italic opacity-50">Localização</p>
                         <p className="text-custom-main text-sm font-[900] uppercase tracking-tight">{user.location || user.Location || "N/A"}</p>
                      </div>
                   </div>

                   <div className="flex items-center gap-6 p-6 bg-white dark:bg-slate-900/40 rounded-3xl border border-slate-200 dark:border-slate-800/50 group transition-all hover:bg-white dark:bg-slate-900/60">
                      <div className="w-14 h-14 bg-slate-50 dark:bg-slate-950 rounded-2xl flex items-center justify-center text-slate-600 group-hover:text-emerald-500 transition-colors border border-slate-200 dark:border-slate-800">
                        <Briefcase size={24} strokeWidth={2.5} />
                      </div>
                      <div className="space-y-1">
                         <p className="text-[9px] font-black text-slate-500 uppercase tracking-[0.4em] italic opacity-50">Cargo / Função</p>
                         <p className="text-custom-main text-sm font-[900] uppercase tracking-tight">{user.cargo || user.Cargo || "OPERADOR LOGÍSTICO"}</p>
                      </div>
                   </div>
                </div>
             </div>
