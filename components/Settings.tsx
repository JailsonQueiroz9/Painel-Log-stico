...
const Settings: React.FC<SettingsProps> = ({ theme, users, onRefreshUsers }) => {
  ...
  const handleToggleStatus = async (user: User) => {
    ...
    try {
      ...
      const permsString = (user.allowedViews || []).map(v => viewMapping[v]).join('; ');
      ...
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-5">
            <div className="p-4 bg-primary-600/10 rounded-2xl border border-primary-500/20 text-primary-500 shadow-xl">
              <Shield size={28} />
            </div>
            <div>
          ...
          <div className="flex items-center gap-4">
            <div className="relative group w-full md:w-80">
              <Search size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-700 group-focus-within:text-primary-500 transition-colors" />
              <input 
                type="text" 
                placeholder="BUSCAR OPERADOR..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-custom-main rounded-xl pl-12 pr-4 py-3 text-[10px] font-black focus:outline-none focus:border-primary-500 transition-all uppercase tracking-widest placeholder:text-slate-800"
              />
            </div>
            <button 
        ...
        <div className="space-y-4">
          {filteredUsers.map((u) => (
            <div 
              key={u.id} 
              className="group bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-900 rounded-[32px] p-6 flex flex-col lg:flex-row lg:items-center justify-between gap-8 hover:bg-white/5 transition-all shadow-xl"
            >
              {/* LADO ESQUERDO: Perfil do Operador */}
              <div className="flex items-center gap-6 min-w-[300px]">
                <div className="w-16 h-16 bg-primary-600 rounded-2xl flex items-center justify-center text-white text-2xl font-black shadow-lg shadow-primary-600/20 shrink-0">
                  {(u.name || u.USUÁRIO || '?').charAt(0).toUpperCase()}
                </div>
                
                <div className="flex flex-col gap-1">
                  <span className="text-base font-black text-custom-main uppercase tracking-tight">
                    {u.name || u.USUÁRIO}
                  </span>
              ...
              <div className="flex items-center justify-between lg:justify-end gap-6 border-t lg:border-t-0 border-slate-200 dark:border-slate-900 pt-4 lg:pt-0">
                <div className="flex flex-col items-end gap-1 px-4">
                  <span className={`text-[9px] font-black uppercase tracking-widest ${u.role === 'admin' ? 'text-primary-500' : 'text-slate-600'}`}>
                    {u.role === 'admin' ? 'NÍVEL: ADMIN' : 'NÍVEL: USER'}
                  </span>
                  <button 
                    onClick={() => handleToggleStatus(u)}
                    disabled={togglingUserId === u.id}
                    className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded border transition-all hover:scale-105 active:scale-95 disabled:opacity-50 ${u.status === 'ativo' ? 'text-emerald-500 border-emerald-900/30 hover:bg-emerald-500/10' : 'text-rose-500 border-rose-900/30 hover:bg-rose-500/10'}`}
                ...
                <div className="flex items-center gap-2">
                  <button 
                    ...
                    className="p-3.5 text-slate-600 hover:text-primary-500 hover:bg-primary-500/10 rounded-2xl border border-transparent hover:border-primary-500/20 transition-all"
                  >
                    <Edit3 size={20} />
                  </button>
                  <button 
          ...
          {filteredUsers.length === 0 && (
            <div className="py-24 text-center border-2 border-dashed border-slate-200 dark:border-slate-900 rounded-[32px]">
...
const PermissionModule = ({ active, icon, label }: { active?: boolean, icon: any, label: string }) => (
  <div className={`
    ...
    ${active 
      ? 'bg-primary-600/5 border-primary-600 text-primary-500 shadow-[0_0_15px_rgba(37,99,235,0.3)] scale-100' 
      : 'bg-transparent border-slate-200 dark:border-slate-800/60 text-slate-800 opacity-30'}
  `}>
